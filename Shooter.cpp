    ...
    if (inGame) {
        ...
        if (waitMouseRelease) {
            if (!mouse->isButtonPressed(sf::Mouse::Left)) {
                waitMouseRelease = false; // кнопку отпустили
            }
            return;
        }
        ...
        if (blockPlayerInput) {
            ...
            return; // ⛔ 1 кадр без ввода
        }

        screen->setTitle(ShooterConsts::PROJECT_NAME);
        playerController->update();   // ✅ ТОЛЬКО ЗДЕСЬ
    } else {
        mainMenu.update();
    }
...
void Shooter::drawPlayerStats() {
    ...
    screen->drawTetragon(Vec2D{xPos, yPos},
                         ...
                         {static_cast<sf::Uint8>((ShooterConsts::HEALTH_MAX - player->health()) /
                          ...
                          static_cast<sf::Uint8>(player->health() * 255 / ShooterConsts::HEALTH_MAX), 0, 255});
